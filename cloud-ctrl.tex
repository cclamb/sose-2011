...
Finally, if needed the \textit{Resource Allocator} will dispatch messages to the \textit{Cluster Controller}, \textit{Nodes}, and \textit{Storage Controller} adjusting system profiles to ensure they remain within acceptable performance ranges.
\\
\\
...
The new system has new associations and components required to implement the degree of control required to limit the accessibility of the network stream.  We have added \textit{Cache Controllers} and \textit{Switches}, defined as:
...
%(what; what does the mp case look like? what are the domains? how does it differ from previous cases?)
% UML
%(why, who, when; motivation; why is the mp case important? to whom? when wi)
%(how; how do we control the systems? differences in perspectives?)	
